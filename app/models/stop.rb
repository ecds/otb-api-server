...
class Stop < ContentBase
  ...
  scope :by_slug_and_tour, lambda { |slug, tour_id|
    joins(:stop_slugs).joins(:tours).where('stop_slugs.slug = ?', slug).where('tour_stops.tour_id = ?', tour_id)
  }
  ...
  def default_values
    self.meta_description ||= HtmlSanitizer.accessible_truncated(description)
  end
