  ...
  describe 'POST #create' do
    context 'with valid params' do
      it 'return 401 when unauthenticated' do
        post :create,
          params: { data: { type: 'tours', attributes: { title: 'Burrito Tour' } }, tenant: TourSet.first.subdir }
    ...
    context 'with valid params' do
      it 'return 401 when unauthenticated' do
        ...
        post :update,
          params: {
            id: tour.id,
            data: { type: 'tours', attributes: { title: 'Burrito Tour' } },
            tenant: TourSet.first.subdir,
          }
        expect(response.status).to(eq(401))
      end
