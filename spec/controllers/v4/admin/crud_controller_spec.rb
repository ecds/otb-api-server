...
RSpec.describe V4::Admin::CrudController, type: :controller do
  describe "POST #create" do
    ...
    it "uploads a map icon" do
      ...
      post :create, params: { tenant: TourSet.second.subdir, model: "map_icon", map_icon: { file:, filename: "map_icon.jpg" } }
      expect(response).to have_http_status(:created)
    end
  ...
  describe "PUT #update" do
    ...
    it "uploads and replaces file" do
      user = create(:user, super: true)
      signed_cookie(user)
      medium = create(:medium, base_sixty_four: nil, file: fixture_file_upload(Rails.root.join("spec", "factories", "images", "atl.png"), "image/png"))
      expect(medium.file.attached?)
    ...
    it "adds a logo to a tour set" do
      ...
      expect(v4_json[:attached]).to be_truthy
